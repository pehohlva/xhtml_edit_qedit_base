...
Edit_html::Edit_html() {
  ...
  connect(this, SIGNAL(customContextMenuRequested(QPoint)), this,
          SLOT(RContext(QPoint)));
  connect(wtext, SIGNAL(customContextMenuRequested(QPoint)), this,
          SLOT(RContext(QPoint)));
  ...
  connect(fontname, SIGNAL(activated(const QString &)), this,
          SLOT(TextFamily(const QString &)));
  connect(wtext, SIGNAL(currentCharFormatChanged(const QTextCharFormat &)),
  ...
  connect(grp, SIGNAL(triggered(QAction *)), this,
          SLOT(MaketextAlign(QAction *)));

  actionAlignLeft =
      new QAction(QIcon(":/img/textleft.png"), tr("Text align left"), grp);
  actionAlignLeft->setShortcut(Qt::CTRL + Qt::Key_S);
  actionAlignLeft->setCheckable(true);
  actionAlignRight =
      new QAction(QIcon(":/img/textright.png"), tr("Text align right"), grp);
  actionAlignRight->setShortcut(Qt::CTRL + Qt::Key_D);
  actionAlignRight->setCheckable(true);
  actionAlignCenter =
      new QAction(QIcon(":/img/textcenter.png"), tr("Text align center"), grp);
  actionAlignCenter->setShortcut(Qt::CTRL + Qt::Key_E);
  actionAlignCenter->setCheckable(true);
  actionAlignJustify = new QAction(QIcon(":/img/textjustify.png"),
                                   tr("Text align Justify"), grp);
  ...
  connect(fontsize, SIGNAL(activated(const QString &)), this,
          SLOT(TextSize(const QString &)));
  fontsize->setCurrentIndex(
      fontsize->findText(QString::number(QApplication::font().pointSize())));
  ...
  connect(vol_underline, SIGNAL(clicked(bool)), wtext,
...
void Edit_html::SaveAsFile() {
  ...
  bool tclean;
  QString s =
      QFileDialog::getSaveFileName(this, tr("Select place to export file html"),
                                   QString(setter.value("LastDir").toString()),
                                   "Plain text (*.xhtml *.html)");

  QString msgDB = tr("You Like to Tidy File? (Clean conform)");
  int removeyes =
      QMessageBox::question(this, tr("Pleas Confirm!"), msgDB, tr("&Yes"),
                            tr("&No"), QString(), 8888, 9999);
...
void Edit_html::TextFamily(const QString &f) { wtext->setFontFamily(f); }
...
void Edit_html::MakefilePrintPdf() {
  QString fileName = QFileDialog::getSaveFileName(
      this, "Export PDF", QString(setter.value("LastDir").toString()), "*.pdf");
  if (fileName.size() > 1) {
    ...
    /////////////QMessageBox::information( this, tr( "File info." ),tr("Il file
    ///? stato salvato"));
    if (is_file(fileName)) {
      ...
      if (!r) {
        QMessageBox::warning(this, tr("Error"),
                             tr("Unable to open file or dir %1").arg(trafile));
      }
    }
  }
  QApplication::restoreOverrideCursor();
...
void Edit_html::RContext(const QPoint &pos) {
  ...
  TContext->addAction(tr("Open other xhtml or html file"), this,
                      SLOT(OpenNewDoc()));
  ...
  if (bellavista) {
    ...
    TContext->addAction(QIcon(QString::fromUtf8(":/img/textbold.png")),
                        tr("Text Bold"), vol_bold, SLOT(click()));
    TContext->addAction(QIcon(QString::fromUtf8(":/img/textitalic.png")),
                        tr("Text italic"), vol_italic, SLOT(click()));
    TContext->addAction(QIcon(QString::fromUtf8(":/img/textunder.png")),
                        tr("Text underline"), vol_underline, SLOT(click()));
    TContext->addAction(QIcon(QString::fromUtf8(":/img/link.png")),
                        tr("Link or UnLink selected Text (mailto,http)"),
                        vol_link, SLOT(click()));
    TContext->addAction(QIcon(QString::fromUtf8(":/img/document.png")),
                        tr("Line Break"), vol_br, SLOT(click()));

    TContext->addSeparator();
    TContext->addAction(QIcon(QString::fromUtf8(":/img/textleft.png")),
                        tr("Text align Left"), vol_left, SLOT(click()));
    TContext->addAction(QIcon(QString::fromUtf8(":/img/textright.png")),
                        tr("Text align Right"), vol_right, SLOT(click()));
    TContext->addAction(QIcon(QString::fromUtf8(":/img/textcenter.png")),
                        tr("Text align Center"), vol_center, SLOT(click()));
    TContext->addAction(QIcon(QString::fromUtf8(":/img/textjustify.png")),
                        tr("Text align Justify"), vol_justify, SLOT(click()));
    TContext->addSeparator();
  }
  if (!bellavista) {
    ...
    TtableContext->addAction(tr("Insert Table here"), this,
                             SLOT(CreateanewTable()));
    if (lastcursor.currentTable()) {
      TtableContext->addAction(
          QIcon(QString::fromUtf8(":/img/table.png")),
          tr("Table (this) Propriety BackgroundColor, Padding, Spacing"), this,
          SLOT(TableSetting()));
      TtableContext->addAction(QIcon(QString::fromUtf8(":/img/row_table.png")),
                               tr("Merge selected cell (if select)"), this,
                               SLOT(MergeCellByCursorPosition()));
      TtableContext->addAction(tr("Append Row on this table"), this,
                               SLOT(AppendTableRows()));
      TtableContext->addAction(tr("Append Cools on this table"), this,
                               SLOT(AppendTableCools()));
      TtableContext->addAction(tr("Remove this row"), this,
                               SLOT(RemoveRowByCursorPosition()));
      TtableContext->addAction(tr("Remove this cool"), this,
                               SLOT(RemoveCoolByCursorPosition()));
      TtableContext->addAction(tr("Set Cell backgroundColor"), this,
                               SLOT(SetTableCellColor()));
      TtableContext->addAction(tr("Set Cell Width"), this,
                               SLOT(SetColumLarge()));
    }
  ...
  if (isqtextblok) {
    ...
    TContext->addAction(tr("Changes Text Margin \"%2px\" to %1..")
                            .arg(texticurrent)
                            .arg(formatibb.topMargin()),
                        this, SLOT(SetTextBlockMargin()));
    TContext->addSeparator();
  }
...
void Edit_html::SetTextBlockMargin() {
  ...
  if (isqtextblok) {
    ...
    bool ok;
    qreal LargeMargin = QInputDialog::getInt(
        this, tr("Set Text Margin to %1").arg(texticurrent), tr("Pixel:"),
        formatibb.topMargin(), 0, 300, 1, &ok);
    if (LargeMargin < 301 && ok) {
      ////////////////bottomMargin(), setTopMargin(), setLeftMargin(), and
      ///setRightMargin().

      //////qDebug() << "### formatibb.leftMargin() " << formatibb.leftMargin();
      ////qDebug() << "### formatibb.rightMargin() " << formatibb.rightMargin();
      //////qDebug() << "### formatibb.bottomMargin() " <<
      ///formatibb.bottomMargin();
