...
#define IMM_BUILD QString("%1pics/").arg(WORK_CACHEDIR)

#define CURL_DIR QString("%1curl/").arg(WORK_CACHEDIR)
#define IMAGE_TMP_DIR QString("%1imgtmp/").arg(WORK_CACHEDIR)

#define LOGCURLSTDOUT QString("%1curl.log").arg(CURL_DIR)

#define _BASEDBDIR_ QString("%1user/").arg(WORK_CACHEDIR)
#define APPLICATION_SETTING QString("%1config.xml").arg(WORK_CACHEDIR)

#define APPLICATION_LOGS QString("%1/logfile.txt").arg(WORK_CACHEDIR)
#define UNZIPDIR QString("%1tmp/").arg(WORK_CACHEDIR)
#define _XML_STREAMS_ QString("%1/streams.xml").arg(WORK_CACHEDIR)

#define DEBUGFILE QString("%1/saypath.txt").arg(QDir::homePath())

#define LOGSQLERROR QString("%1curl.log").arg(QDir::currentPath())
