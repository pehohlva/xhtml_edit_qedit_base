...
public:
  ...
  bool SetUp_xhtml_full_page() {
    ...
    if (ok) {
      ...
      status = tidyLoadConfig(
          doc,
          configfileti
              .data()); /* http://ch2.php.net/manual/de/function.tidy-load-config.php
                         */
      if (status != 0) {
  ...
  QString TidyExternalHtml(QString body) {
    QString prehtml =
        TidyCleanhtml(body); /* base clean to stay on minimal standard xhtml */
    QStringList notneed;
    notneed.clear();
    ///////////////width="456" lang  class
    QRegExp expression("width=[\"\'](.*)[\"\']",
                       Qt::CaseInsensitive); /* table td tr width image amen */
  ...
  QString CleanTagQt4(QString body, QString class_name) {
    ...
    newbody.replace(QRegExp("<div[^>]*>([^<]*)</div>", Qt::CaseInsensitive),
                    "\\1 <!-- QRegExp -->");
    ...
    QString tname =
        QString("%1, %2 %3 %4 +0200")
            .arg(day_en, QString::number(timecute.day()), month_en, last);
    finale = TidyCleanhtml(
        QString("<div xmlns=\"http://www.w3.org/1999/xhtml\" "
                "class=\"####CLASSNAME####\" title=\"Programm powered by "
                "PPK-Webprogramm www.ciz.ch document from %2 \">%1</div>")
            .arg(wbody)
            .arg(tname));
    finale.replace("\n", " ");
...
public:
  ...
  bool image_extension(QString nametomake) {
    ...
    QString na;
    na = nametomake.toLower();
    if (na.endsWith(".png") or na.endsWith(".jpeg") or na.endsWith(".jpg") or
        na.endsWith(".gif")) {
      fanny = true;
    }
    return fanny;
  ...
  QString BrowserDir(uint unixtime, QString base) {
    QString base_3 = "";
    QString base_0 =
        QString("/%1/%2/").arg(base, datetr("yyyy", (uint)unixtime));
  ...
  QString UmanTimeFromUnix(uint unixtime) {
    ...
    QStringList RTFdays = QStringList() << "day_NULL"
                                        << "Mon"
                                        << "Tue"
                                        << "Wed"
                                        << "Thu"
                                        ...
                                        << "Sat"
                                        << "Sun";
    QStringList RTFmonth = QStringList() << "month_NULL"
                                         << "Jan"
                                         << "Feb"
                                         << "Mar"
                                         << "Apr"
                                         ...
                                         << "Nov"
                                         << "Dec";
    QDate timeroad(dateswap("yyyy", unixtime), dateswap("M", unixtime),
                   dateswap("d", unixtime));
  ...
  QString sql_Quote(QString xml) {
    ...
    // strings to XML. see
    // http://hdf.ncsa.uiuc.edu/HDF5/XML/xml_escape_chars.html for details
    QString text = xml;
    text.replace("\"", "\\\""); /* 228 ``*/
    text.replace("'", "`");
    return text;
    ...
    if (dir.exists()) {
      ...
      for (int l = 0; l < list.size(); l++) {
        ...
        if (fi.isDir() && fi.fileName() != "." && fi.fileName() != "..")
          ...
          ///////if (!ret) {
          ////////SqlLog("Can't remove: " + fi.absoluteFilePath() + "
          ///(write-protect?)");
        }
      }
      /////// SqlLog("Remove: " + d + " ");
  ...
  void ResetDir() { QDir::setCurrent(currentdir); }
