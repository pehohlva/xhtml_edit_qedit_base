...
QString item = QInputDialog::getItem(this, tr("Table Setting"), tr("Align:"),
xalign, setfoundalign , false, &ok); int newalign = BraketNummer(item); qDebug()
<< "### table alignment " << formtab.alignment(); qDebug() << "### new table
alignment " << newalign;

QColor normalcolor = xbgcolor.color();
qreal paddx = QInputDialog::getDouble(this, tr("Table Setting"),
tr("CellPadding:"), xpadding , 0, 10, 1, &ok); qreal spacix =
QInputDialog::getDouble(this, tr("Table Setting"), tr("CellSpacing:"), xspacing
, 0, 10, 1, &ok1); QColor cole = QColorDialog::getColor(normalcolor, this); if
(ok and ok1) { formtab.setCellPadding(paddx); formtab.setCellSpacing(spacix); if
(cole.isValid()) { QBrush newcolorsett(cole,Qt::SolidPattern);
        formtab.setBackground(newcolorsett);
        }
        if (newalign > 0) {
            if (newalign == 1) {
