...
{
     bool ok;
     int i = QInputDialog::getInt(this, tr("Wait Sec. to PrintScreen...."),
...
connect(spinBox_2, SIGNAL(valueChanged(int)), this, SLOT(misurecambiaL()));
  ...
  */  
    ...
    imgcoo->setText(tr("Init crop on X=%1 Y=%2  px %3x%4 CTRL + mouse to move crop area.").arg(Top_startX).arg(Top_startY).arg(TagliaPoi.width()).arg(TagliaPoi.height())); 
    } else {
...
void Interface::keyPressEvent(QKeyEvent *e)
{
    ...
    {
    QApplication::clipboard()->setImage(display.toImage(),QClipboard::Clipboard);
    return; 
    }
