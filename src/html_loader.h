...
class Html_Loader : public QObject, public Base_Modul {
  ...
  //
public:
  Html_Loader(QString stream, const QString imagetmpdir,
...
private:
  ...
  void Load_Image_Connector() {
    ...
    while ((iPosition = expression.indexIn(html, iPosition)) != -1) {
      ...
      ////////////////////qDebug() << "### canna MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
      ///" << canna << " img->" << semi1;
  ...
  void Register_File() {
    ...
    for (int i = 0; i < remotefiletolocal; ++i) {
      ...
      if (is_file(localfile)) {
        ...
        doc->addResource(
            QTextDocument::ImageResource, QUrl(filefullpath),
            QPixmap(QString("%1%_%2").arg(registername).arg(imposi)));
        /////////////////////////qDebug() << "### register downloaded file " <<
        ///filefullpath;
      }
    }
  ...
  void AppendImage(const QString foundfile) {
    ...
    if (shema != "http" or shema != "ftp") {
      if (localrec.exists(ifile)) {
        ...
        if (!filefullpath.contains(image_tmp_path)) {
          filename =
              QString("%1%2").arg(image_tmp_path).arg(locinfo.fileName());
        ...
        doc->addResource(
            QTextDocument::ImageResource, img2,
            QPixmap(QString("%1%_%2").arg(registername).arg(imposi)));
      }
    }
    ...
    if (shema == "http" or shema == "ftp") {
      if (img.isValid()) {
        ...
        filename =
            QString("%1%2").arg(image_tmp_path).arg(geturl.replace(path, ""));
        rmrfimage.append(filename);
        html = html.replace(ifile, filename);
        int numera = NeededFile.size();
        //////////////////qDebug() << "###
        ///POOhttpOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO   " << numera;
  ...
  int StartGetFile(QString fileremote, QString localfile, int actposizion) {
    ...
    actualurlerror = QString("Unable to load url %1... to local %2")
                         .arg(fileremote)
                         .arg(localfile);
    ...
    request.setRawHeader("Accept-Language",
                         "it,de-de;q=0.8,it-it;q=0.6,en-us;q=0.4,en;q=0.2");
    ...
    connect(netmanager, SIGNAL(finished(QNetworkReply *)), this,
            SLOT(downloadEndSave(QNetworkReply *)));
    ...
    if (!logFile->open(QIODevice::ReadWrite)) {
      errorlist.append(QString("Unable to write on file %2 from remote url %1")
                           .arg(fileremote)
                           .arg(localfile));
...
public slots:
  ...
  void LoadPosition(int now) {
    ...
    //////////////qDebug() << "### actual get on ->" << now << " sumsize->"  <<
    ///bigmap;
  ...
  void Method_Get_Finish(int requestId, bool error) {
    ...
    ///////////////qDebug() << "### GrabMapidfinisch  " << requestId <<  "
    ///error="  << error;
    if (error) {
      logFile->remove();
